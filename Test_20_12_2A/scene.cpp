...
void Scene::GetRangeSet(Object *object, ObjMap *set) {
    ...
    if (object->x_pos != obj_x_list_.end()) {
        ...
        while (1) {
            ...
            if (abs(object->x - (*iter)->x) > distance) {
                break;
            }
            x_set[(*iter)->id] = *iter;
        }
    ...
    if (object->x_pos != obj_x_list_.begin()) {
        ...
        while (1) {
            --iter;
            if (abs((*iter)->x - object->x) > distance) {
                break;
            }
            x_set[(*iter)->id] = *iter;
            if (iter == obj_x_list_.begin()) {
    ...
    if (object->y_pos != obj_y_list_.end()) {
        ...
        while (1) {
            ...
            if (abs(object->y - (*iter)->y) > distance) {
                break;
            }
    ...
    if (object->y_pos != obj_y_list_.begin()) {
        ...
        while (1) {
            --iter;
            if (abs((*iter)->y - object->y) > distance) {
                break;
            }
            if (x_set.find((*iter)->id) != x_set.end()) {
                (*set)[(*iter)->id] = *iter;
...
void Scene::PrintList() {
	cout << "x:" << endl;
	for (auto x_obj : obj_x_list_) {
		cout << x_obj->id << "(" << x_obj->x << "," << x_obj->y << ") ";
	}
	cout << endl;

	cout << "y:" << endl;
	for (auto y_obj : obj_y_list_) {
		cout << y_obj->id << "(" << y_obj->x << "," << y_obj->y << ") ";
	}
	cout << endl;
}

void Scene::PrintAOI(int id) {
	ObjMap::iterator obj_iter = obj_set_.find(id);
	if (obj_iter == obj_set_.end()) {
		return;
	}
	...
	GetRangeSet(object, &aoi_set_);

	if (0 < aoi_set_.size()) {
		cout << id << "(" << object->x << "," << object->y << ")->";
		for (auto obj : aoi_set_) {
			cout << obj.first << "(" << obj.second->x << "," << obj.second->y << ") ";
		}
		cout << endl;
	}
